...
type ConfigKV[T int | uint32 | bool | string] struct {
	store sync.Map
}

func NewConfigKV[T int | uint32 | bool | string]() *ConfigKV[T] {
	return &ConfigKV[T]{}
}
...
func (c *ConfigKV[T]) List() []string {
	...
	c.store.Range(func(k, v any) bool {
		...
		case reflect.Uint32:
			cons = strconv.Itoa(int(v.(uint32)))
		case reflect.String:
			cons = v.(string)
		case reflect.Bool:
